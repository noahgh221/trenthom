...
<!-- Loads all ead files in directory and creates tabbed file of info -->
    ...
    <xsl:for-each select="collection('file:/C:/Users/nh48/Documents/GitHub/trenthom/4_schema_eads/?select=*.xml')">
        ...
        <xsl:for-each select="//ead:controlaccess//ead:persname|
         ...
         <xsl:value-of select="$tab"/>
        <xsl:value-of select="local-name()"/> <!-- type of heading (persname, corpname, etc.) -->
        <xsl:value-of select="$tab"/>
